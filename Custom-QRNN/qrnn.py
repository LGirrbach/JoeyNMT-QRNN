...
from ifo_pooling import IFOPooling
from f_pooling import FPooling
...
class QRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers=1,
        ...
        for layer in range(num_layers):
            ...
            # For dense convolutions/residual connections
            # update the input size to each layer accordingly
            if dense:
    ...
    def forward(self, input, hx=None):
        ...
        for layer in range(self.num_layers):
            ...
            for direction in range(self.num_directions):
                ...
                # input_prime: (batch, features, time)
            ...
            # Dense connections
            input_prime = torch.cat(directions, dim=1)  # Concatenate directions
                                                        # along feature dim
...
class QRNNLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        training = self.training  # Save training variable
        if self.reverse:
            input = input.flip(2)
        
        # Prepend padding for masked convolutions
        padding = torch.zeros(
    ...
    def f_pooling(self, x, z, f):
        """ F-Pooling: Now we have a separate class for this"""
        #h = [(1-f[:, :, 0]) * z[:, :, 0]]
    ...
    def fo_pooling(self, x, z, f):
        """ FO-Pooling: Now we have a separate class for this"""
        o = torch.sigmoid(self.o_conv(x))  # But not for convolutions
    ...
    def ifo_pooling(self, x, z, f):
        """ IFO-Pooling: Now we have a separate class for this"""
        o = torch.sigmoid(self.o_conv(x))  # But not for convolutions
