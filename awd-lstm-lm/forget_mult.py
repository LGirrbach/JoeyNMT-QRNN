...
class GPUForgetMult(torch.autograd.Function):
    ...
    def forward(ctx, f, x, hidden_init=None):
        ...
        GPUForgetMult.forget_mult(grid=grid, block=(grid_hidden_size, 1),
    ...
    def backward(ctx, grad_h):
        ...
        grid_hidden_size = min(hidden_size, 512)
        grid = (math.ceil(hidden_size / grid_hidden_size), batch_size)
        GPUForgetMult.forget_mult_backward(grid=grid, block=(grid_hidden_size, 1),
...
class ForgetMult(torch.nn.Module):
    ...
    def forward(self, f, x, hidden_init=None, use_cuda=True):
        ...
        if hidden_init is None: return self.gpu_forget_mult(f, x) if use_cuda else self.cpu_forget_mult(f, x)
        return self.gpu_forget_mult(f, x, hidden_init) if use_cuda else self.cpu_forget_mult(f, x, hidden_init)
...
if __name__ == '__main__':
    ...
    a      = torch.rand(seq, batch, hidden, requires_grad=True, device="cuda", dtype=torch.float32)
    forget = torch.rand(seq, batch, hidden, requires_grad=True, device="cuda", dtype=torch.float32)
    last_h = torch.rand(batch, hidden, requires_grad=True, device="cuda", dtype=torch.float32)
    #a      = torch.rand(batch, hidden, requires_grad=True, device="cuda", dtype=torch.float32)
    #forget = torch.rand(batch, hidden, seq, requires_grad=True, device="cuda", dtype=torch.float32)
    #last_h = torch.rand(batch, hidden, requires_grad=True, device="cuda", dtype=torch.float32)
