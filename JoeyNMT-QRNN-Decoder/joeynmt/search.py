...
def beam_search(
    ...
    encoder_hidden = tile(encoder_hidden.contiguous(), size, dim=1)
    ...
    ### tile encoder states and decoder initial states beam_size times
        ...
        #hidden = tile(hidden[0], size, dim=1), tile(hidden[1], size, dim=0)  # layers x batch*k x dec_hidden_size
    ...
    for step in range(max_output_length):
        ...
        encoder_hidden = encoder_hidden.index_select(1, select_indices)
        ...
        if hidden is not None and not transformer:
            if isinstance(hidden, tuple):
                ...
                (h, c), input = hidden
                h = [h_.index_select(0, select_indices) for h_ in h]
                c = [c_.index_select(0, select_indices) for c_ in c]
                #h = h.index_select(1, select_indices)
                #c = h.index_select(1, select_indices)
                input = input.index_select(0, select_indices)
                hidden = (h, c), input
            else:
