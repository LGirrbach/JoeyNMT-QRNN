...
class _SoftmaxLoss(torch.nn.Module):
    ...
    def forward(self, embeddings: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        return torch.nn.functional.nll_loss(probs, targets.long(), reduction="sum"), probs
    
    def get_probs(self, embeddings: torch.Tensor):
        probs = torch.nn.functional.log_softmax(
                torch.matmul(embeddings, self.softmax_w) + self.softmax_b,
                dim=-1
        )
...
class LanguageModel(Model):
    ...
    def _compute_loss(self,
        ...
        if self._bidirectional:
            ...
            backward_loss, backward_probs = self._loss_helper(1, backward_embeddings, backward_targets, token_embeddings)
        else:
            forward_embeddings = lm_embeddings
            backward_loss = None
            backward_probs = None

        forward_loss, forward_probs = self._loss_helper(0, forward_embeddings, forward_targets, token_embeddings)
        return forward_loss, backward_loss, forward_probs, backward_probs
    ...
    def forward(self,  # type: ignore
        ...
        contextual_embeddings, hidden = self._contextualizer(
                embeddings, mask, hidden_state=hidden
        )
        ...
        if not training:
            forward_probs = self._softmax_loss.get_probs(self._dropout(contextual_embeddings))
            return_dict.update({
        ...
        if token_ids is not None:
            ...
            forward_loss, backward_loss, forward_probs, backward_probs = \
                self._compute_loss(contextual_embeddings_with_dropout,
                                   embeddings,
                                   forward_targets,
                                   backward_targets)
            if forward_targets.shape[1] > 1:
                num_targets = torch.sum((forward_targets > 0).long())
            else:
                num_targets = torch.ones(1, device=forward_targets.device).long().sum()
            if num_targets > 0:
                if self._bidirectional:
